...
def _personal_deductible_amount(current_deductibles_model: object):
    return round(current_deductibles_model.base_deductible * \
        current_deductibles_model.personal_deductible_coef, 2)


def _dependents_children(current_deductibles_model: object, employee: object):
...
def _dependents_deductible(current_deductibles_model: object, employee: object):
    return round(employee.no_dependents * 0.7 * \
        current_deductibles_model.base_deductible, 2)


def _disabled_dependents(current_deductibles_model: object, employee: object):
    return round(employee.no_dependents_disabled * 0.4 * \
        current_deductibles_model.base_deductible, 2)


def _disabled_dependents_100(current_deductibles_model: object, employee: object):
    return round(employee.no_dependents_disabled_100 * 0.5 * \
        current_deductibles_model.base_deductible, 2)

def _deductibles(self):
